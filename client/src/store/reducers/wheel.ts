...
import { THEMES, Wheel, WheelItem, wheels, defaultSpinConfig, SpinConfig } from "src/constants";
...
const wheelReducer = (state = initialState, action: ActionTypes): WheelState => {
  switch (action.type) {
    case SET_VOLUME:
    ...
    case SET_SELECTED_WHEEL:
      return { ...state, selectedWheel: action.payload, history: action.payload.options };
    ...
    case SET_RANDOM_INITIAL_ANGLE_OPTION:
      return { 
        ...state, 
        spinConfig: { ...state.spinConfig, randomInitialAngleOption: action.payload } 
      };
